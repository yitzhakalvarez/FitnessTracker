<template>
  <div class="Tracking">
    ...
    <!-- Container for plan selector -->
    <div class="planSelectorContainer">
      <input
        id="planSelectBtn"
        class="btn selectPlanBtn"
        type="button"
    ...
    <div v-if="selectedPlan">
      ...
      <div class="planContainer">
        ...
        />
        <button
          class="btn editAndSavePlanBtn"
          type="button"
          @click="editAndSavePlan()"
        ></button>
      </div>
...
export default {
  ...
  methods: {
    //Get the data the user has saved for the planner
    getFood() {
      api.getMealPlanner_Plan().then(plans => {
        api.getMealPlanner_Meal().then(meals => {
          api.getMealPlanner_Food().then(food => {
            //Sort the plans by orderID and assign planArray
            plans.sort(function(plan1, plan2) {
              return plan1.orderID - plan2.orderID;
            });
            this.planArray = plans;
            if (this.selectedPlan == null && this.planArray.length > 0)
              this.selectedPlan = this.planArray[0];
            ...
            this.proteinPercent =
              Math.round(((totalProtein * 4) / (this.BMR * 0.3)) * 100 * 10) /
              10;
          });
        });
      });
    ...
    //Edit and save button for plans
    editAndSavePlan() {
      //Toggle editPlanActive
      this.editPlanActive = !this.editPlanActive;
      //Edit is true
      if (this.editPlanActive == true) {
        document.getElementById(
          "planName " + this.selectedPlan.ID
        ).disabled = false;
        document.getElementById(
          "planName " + this.selectedPlan.ID
        ).style.outline = "1px lightblue solid";
        document.getElementById("removePlanBtn").style.visibility = "visible";
      }
      //Edit is false
      else {
        document.getElementById(
        ...
        document.getElementById("removePlanBtn").style.visibility = "hidden";
        var planName = document.getElementById(
          "planName " + this.selectedPlan.ID
        ).value;
        api.editMealPlanner_Plan(
          this.selectedPlan.ID,
          this.selectedPlan.orderID,
          planName
        );
      }
    },
    //Edit and save button for meals
    editAndSaveMeal(meal) {
      //Toggle editActive
    ...
    //Add a plan
    addPlan() {
      var orderID = this.planArray.length;
      api.insertMealPlanner_Plan(orderID, "").then(planInserted => {
        if (planInserted != null) {
          this.selectedPlan = planInserted;
          this.editPlanActive = false;
          this.getFood();
        }
      });
    },
    //Remove plan, meal and food from database by ID
    removePlan() {
      //Remove meal
      api.removeMealPlanner_Plan(this.selectedPlan.ID).then(planRemoved => {
        if (planRemoved == true) {
          //Remove meals that was inside the plan
          var mealArray = this.mealArray;
    ...
    //Add a meal
    addMeal() {
      var planID = this.selectedPlan.ID;
      var orderID = this.mealArray.length;
      api.insertMealPlanner_Meal(planID, orderID, "").then(mealInserted => {
        if (mealInserted == true) this.getFood();
      });
    },
    //Remove meal and food from database by ID
    removeMeal(meal) {
      //Remove meal
      api.removeMealPlanner_Meal(meal.ID).then(mealRemoved => {
      ...
      for (var i = 0; i < meal.foodArray.length; i++) {
        ...
        api.removeMealPlanner_Food(foodID);
      }
    },
    //Add food
    addFood(meal) {
    ...
    removeFood(ID) {
      api.removeMealPlanner_Food(ID).then(foodRemoved => {
...
.Tracking .headerContainer .header .line {
  ...
  border-bottom: 3px rgb(141, 111, 111) solid;
}
...
.Tracking .tracked-row {
  ...
  padding: 10px;
  text-align: center;
  border-top: 3px rgb(36, 32, 32) solid;
  border-bottom: 3px rgb(36, 32, 32) solid;
}
